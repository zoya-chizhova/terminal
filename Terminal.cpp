...
void Terminal::run() {
   std::cout << "Virtual Terminal started."
...
void Terminal::handleCat(std::istringstream& iss) {
   ...
   if (entry && !entry->isHidden()) {
      ...
      std::cout << "File not found or hidden: " 
               << fileName 
               << "\n";
   }
}
void Terminal::handleTouch(std::istringstream& iss) {
   ...
   if (!fileName.empty()) {
      auto newFile = 
            std::make_unique<SingleLineFile>(fileName);
...
void Terminal::handleMkdir(std::istringstream& iss) {
   ...
   if (!dirName.empty()) {
      auto newDir = 
            std::make_unique<Directory>(dirName);
   ...
   if (entry) {
      ...
      if (it != currentDir->getChildren().end()) {
         hiddenEntries.push_back(
            std::move(
                  const_cast<std::unique_ptr<Entry>&>(*it)
            )
         );
         currentDir->removeChild(entryName);
         std::cout << "Entry hidden: " 
                  << entryName 
                  << "\n";
      }
   } else {
...
void Terminal::handleRestore(std::istringstream& iss) {
   ...
   if (it != hiddenEntries.end()) {
      ...
      std::cout << "Hidden entry not found: " 
               << entryName 
               << "\n";
   }
}
